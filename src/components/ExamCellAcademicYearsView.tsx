...
export const ExamCellAcademicYearsView: React.FC = () => {
  const { academicYearsList, masterDataLoading, refreshMasterData } = useApp();
  ...
  const filteredYears = useMemo(() => {
    if (!searchQuery.trim()) return academicYearsList;
    const q = searchQuery.toLowerCase().trim();
    return academicYearsList.filter(y =>
      y.year_label.toLowerCase().includes(q) ||
      y.status.toLowerCase().includes(q)
    );
  }, [academicYearsList, searchQuery]);
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[#E5E7EB] pb-6">
        <div>
          ...
          <p className="mt-1 text-xs text-[#64748B]">
            Registered academic sessions and cohorts configured in the institutional database.
          </p>
        </div>
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-blue-100 bg-blue-50/70 p-4 text-xs text-blue-900">
        ...
        <div>
          <span className="font-bold">Managed by Super Administration:</span> Academic year sessions are configured globally. Active sessions are automatically made available in the Question Paper Generation Wizard and Question Bank Import workflows.
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-[#E5E7EB] bg-white overflow-hidden shadow-xs">
        {masterDataLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            <h3 className="text-sm font-bold text-[#111827]">No academic years found</h3>
            <p className="mt-1 text-xs text-[#64748B]">
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-[#E5E7EB] bg-[#F7F8FA] text-[10px] font-extrabold uppercase tracking-wider text-[#64748B]">
                  <th className="py-3.5 px-4 sm:px-6">Academic Year</th>
                  <th className="py-3.5 px-4">Session Span</th>
              ...
              <tbody className="divide-y divide-[#E5E7EB]">
                {filteredYears.map(yr => (
                  <tr key={yr.id} className="hover:bg-[#F9FAFB] transition-colors">
                    <td className="py-4 px-4 sm:px-6 font-mono font-black text-sm text-[#111827]">
                      {yr.year_label}
                    </td>
                    <td className="py-4 px-4 text-[#64748B]">
                      {yr.start_year && yr.end_year ? `${yr.start_year} – ${yr.end_year}` : yr.year_label}
                    </td>
                    <td className="py-4 px-4">
                      <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wider ${
                        ...
                        <span className={`h-1.5 w-1.5 rounded-full ${yr.status === 'active' ? 'bg-emerald-500' : 'bg-slate-400'}`} />
                        {yr.status}
                      </span>
                    </td>
                    <td className="py-4 px-4 text-[#94A3B8]">
                      {yr.created_at ? new Date(yr.created_at).toLocaleDateString('en-GB') : '—'}
                    </td>
                  </tr>
                ))}
              </tbody>
